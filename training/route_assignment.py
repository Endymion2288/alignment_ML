...
@dataclass(frozen=True)
class AssignedRouteEvent:
    """Truth-free route-solver output for one physical synthetic event."""
...
def assign_adjacent_route_sets(
    sets: Sequence[object],
    scores: Sequence[np.ndarray],
    ...
    | None = None,
) -> tuple[AssignedRouteEvent, ...]:
    """Run the fixed route solver and retain selections for global fitting.

    This is deliberately separate from metric evaluation: consumers such as
    the physical alignment loop receive only route endpoints, scores, and
    dustbins generated by the frozen candidate graph.  They do not need, and
    must not consume, truth labels used by evaluation helpers.
    """
    if len(sets) != len(scores):
        raise ValueError("candidate sets and scores are not aligned")
    resolved = (
    ...
    if complete_route_scores_by_event is not None:
        expected = {group.key for group in resolved.groups}
        supplied = set(complete_route_scores_by_event)
        if expected != supplied:
            raise ValueError("complete route-score event keys differ from the physical route context")
    result: list[AssignedRouteEvent] = []
    for group in resolved.groups:
        selection = adjacent_route_assignment(
            group.event,
            group.station_matrices,
            config,
            complete_route_scores=(
                None if complete_route_scores_by_event is None else complete_route_scores_by_event[group.key]
            ),
        )
        result.append(
            AssignedRouteEvent(
