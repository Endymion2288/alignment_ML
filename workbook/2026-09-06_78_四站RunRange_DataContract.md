...
已经可以独立于 Condor 数值表成立的合同事实：

| 层 | 结论 | 证据 |
|---|---|---|
| A 几何 payload | **confound（对 name-matched `draw_*`）** | train seed `20260821` vs development seed `20260824`；`iteration_00_reference` / `hard_s3_ry` 数值相同，`draw_00` 只共享名字 |
| A 站平面 z | **identity 上相同** | content_audit：S0/S1/S2/S3 z = −1860.15 / 47.4 / 1237.4 / 2427.4 mm |
| B 重建政策 | **同一政策，不同 run-range 文件** | 两边都是 `s0013-r0022`；scan config 除 `input_xaod` 与 sampling seed 外一致 |
| C 传播配置 | **同一 refit / Acts 合同** | `q_over_p_mode=0`，`physical_geometry_repropagation=true`，同一 `refit_chain` |
| D overlay 配方 | **同一配方** | `synthetic_multitrack.seed=20260813`，同一 missing/hard-fake/tracks_per_event |
| E 选择 | **弱，不是主因** | 两边 `nevents=50`，`min_truth_match_fraction=0.99`，`chi2_gate=25`；accepted 事件数只差 1 |
...
源纯物理文件上的 KS / Wasserstein / quantile / overlap 表由 HTCondor `cluster 1109749` 写出，见

`outputs/mc24_four_station_run_range_data_contract_v1/`.

本 workbook **不编造** 尚未落地的逐特征数字。
...
```text
parent HEAD          = 7c06f81  (Workbook 77)
...
物理 corpus 规模（identity content_audit，已在磁盘上）：

| source | events | tracklets |
|---|---:|---:|
| `100047_00100_00149` reference | 50 | 188 |
| `100047_00350_00399` reference | 49 | 179 |
| `100048_00100_00149` reference | 50 | 189 |
| `100048_00350_00399` reference | 50 | 184 |
...
Artifact：xAOD 文件名 + `physical_scan_config.yaml`。
...
同一 `s0013-r0022`。scan policy 共享：`nevents=50`, `q_over_p_mode=0`, `min_truth_match_fraction=0.99`, `chi2_gate=25`, `refinement_iterations=1`, `four_station_v1`。

若 identity payload 上 tracklet state KS 很大，那是 **事件内容**，不是 rec-tag 变更。数字见 Condor `layer_B_reconstruction.json`。

### C. propagation / material / field — **same tool contract**
...
`q_over_p_mode=0`。identity 上的 residual KS 见 Condor `layer_C_propagation.json`。若 station z 相同而 residual 大，则是 **给定不同 tracklet 的传播结果**，或场/材料真变；不能从文件名猜。

### D. overlay generation — **recipe match; not the physical-file origin**

Artifact：overlay `synthetic_multitrack`：
...
配方相同。Train overlay 是六源池，development 是两源 reserved-blind 池。这解释 UID 计数差异，**不能**解释源纯物理 ROOT 上的特征漂移。

### E. selection / filtering — **not primary**

同一 `nevents` / truth-match / chi2 gate。100047 identity accepted events 50 vs 49。Occupancy 率的正式差见 Condor；预注册阈值 `|Δ occupancy| ≥ 0.10` 才把 E 标成 supported。

决策函数（预注册，不在本 run 上调参）在 `training/run_range_data_contract.decide_shift_sources`。
...
单位测试已证明两表随机家族签名不相交；identity 允许共享。
...
Condor 写入（不覆盖历史 payload bank，不做 refit / overlay）：
...
Hermetic tests：`tests/test_run_range_data_contract.py`（12 passed）。
...
submit      = outputs/mc24_four_station_run_range_data_contract_v1/condor/wb78_run_range_data_contract.sub
```

每个 run 保存 git SHA、resolved config、dataset manifest、environment、output JSON。作业成功后读取：

```text
run_metadata.json
environment.json
shift_source_decision.json
dataset_manifest_v2.json
feature_shift_tables.json
```

未覆盖 WB74/WB75/WB76/WB77 artifact。未改 `baselines/route_assignment.py` 默认 legacy decoder。
...
只有在用户明确授权、并且 Condor `1109749` 的 `shift_source_decision.json` 被读入之后，才可以在下面两者中选一：

1. Arm B：低容量 Explicit Route Energy Model（仍要同一 data contract）；
2. Arm A/B/C representation study（仍 `training_authorized=false` 直到单独授权）。

两者都必须使用 Dataset contract v2，而不是历史 `00350` development 作为 holdout。
