#!/usr/bin/env python3
"""Write the S2J inspect-only HTCondor plan.  Submit only with --submit."""
...
from datasets.three_st_qp_measurement_bending_batch import dump_path_for_source, load_config

PROJECT_ROOT = project_root()
WORKER = PROJECT_ROOT / "scripts" / "run_three_st_qp_measurement_bending_batch_condor.sh"
...
def main() -> int:
    ...
    parser.add_argument(
        "--submit-dir", default="outputs/three_st_qp_measurement_bending_batch_v1/condor"
    )
    ...
    if bool(config.get("do_not_auto_submit_without_smoke")) and args.submit:
        # Caller must have already passed smoke; the flag only blocks implicit submit.
        pass
    ...
    for split in ("batch_construction_sources", "batch_validation_sources"):
        for spec in config["mc_data"][split]:
            ...
            event_output = dump_path_for_source(config, spec["source_id"], "events", campaign)
            if output.exists() or event_output.exists():
                raise FileExistsError(f"refusing to overwrite existing batch dump: {output}")
            ...
            sources.append(spec["source_id"])
    jobs_file = submit_dir / "jobs.txt"
    jobs_file.write_text("\n".join(jobs) + "\n", encoding="utf-8")
    submit_file = submit_dir / "three_st_qp_measurement_bending_batch.sub"
    ...
    payload = {
        ...
        "bending_definition": "WB125_YZ_bending_raw_unchanged",
        "config_sha256": sha256_file(config_path),
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "submit_file": str(submit_file),
        "submitted": False,
    }
    plan_path = submit_dir / "submit_plan.json"
    plan_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"wrote {len(jobs)} jobs to {submit_file}")
    if not args.submit:
        print("manifest only; pass --submit after smoke")
        return 0
    command = [
        "bash",
        "-lc",
    ...
    payload["submit_stdout"] = result.stdout
    payload["submit_stderr"] = result.stderr
    plan_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
