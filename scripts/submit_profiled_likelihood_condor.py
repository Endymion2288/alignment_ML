#!/usr/bin/env python3
"""Submit one HTCondor job per WB87 source for B14M profile dumps."""
...
from datasets.leave_target_out_state_materialization import plugin_library_path
from datasets.profiled_weak_nuisance_likelihood import (
    load_config,
    profile_dump_path_for_source,
)

PROJECT_ROOT = project_root()
WORKER = PROJECT_ROOT / "scripts" / "run_profiled_likelihood_condor.sh"
...
def main() -> int:
    ...
    parser.add_argument(
        "--submit-dir", default="outputs/leave_target_out_dump_v1/b14m_profile/condor"
    )
    ...
    for split in ("construction_sources", "validation_sources"):
        for spec in config["mc_data"][split]:
            output = profile_dump_path_for_source(config, spec["source_id"])
            output.parent.mkdir(parents=True, exist_ok=True)
            xaod = Path(spec["input_xaod"])
            if not xaod.is_file():
                raise FileNotFoundError(f"input xAOD is missing: {xaod}")
            if output.is_file():
                raise FileExistsError(f"refusing to overwrite profile dump: {output}")
            jobs.append(
    ...
    submit_file = submit_dir / "ckf_leave_target_out_profile.sub"
