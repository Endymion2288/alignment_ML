#!/usr/bin/env bash
# Compile the standalone C++ 3ST q/p refit-provenance dump helper.
# Does not modify Calypso CMake, fitter, seed, or geometry.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "$script_dir/.." && pwd)"
src_dir="$project_root/alignment/three_st_qp_refit_provenance_dump"
out_dir="$project_root/build/three_st_qp_refit_provenance_dump"
...
flags = filtered[1:]
flags += [
    '-DATLAS_PACKAGE_NAME="CkfThreeStQpRefitProvenance"',
    "-DCkfThreeStQpRefitProvenance_EXPORTS",
    f"-I{src_dir}",
    f"-I{project_root}",
]
sources = [
    src_dir / "CkfThreeStQpRefitProvenanceAlg.cxx",
    src_dir / "CkfThreeStQpRefitProvenance_entries.cxx",
]
objects = []
...
so = out_dir / "libCkfThreeStQpRefitProvenance.so"
...
cat > "$out_dir/libCkfThreeStQpRefitProvenance.components" <<'EOF'
v2::libCkfThreeStQpRefitProvenance.so:CkfThreeStQpRefitProvenanceAlg
EOF
...
(
  cd "$out_dir"
  "$genconf_bin" -o "$out_dir/genConf" -p CkfThreeStQpRefitProvenance --no-init \
    -i libCkfThreeStQpRefitProvenance.so
)
...
printf '%s\n' "$part" > "$parts_file"
python "$merge_script" "$out_dir/CkfThreeStQpRefitProvenance.confdb2" "$parts_file"
printf 'Built C++ 3ST q/p refit-provenance helper in %s\n' "$out_dir"
ls -l "$out_dir/libCkfThreeStQpRefitProvenance.so" \
  "$out_dir/libCkfThreeStQpRefitProvenance.components" \
  "$out_dir/CkfThreeStQpRefitProvenance.confdb2"
