#!/usr/bin/env python3
"""Read-only xAOD truth-muon kinematics audit for source-data selection.

The tool verifies only ROOT metadata-backed branch names and reports momentum
components in the source file's native units.  It does not make an assumption
about units, reconstruction conventions, or detector geometry, and does not
write a modified xAOD.
"""
...
def main() -> None:
    ...
    with uproot.open(source) as root_file:
        ...
        if missing:
            raise ValueError("required truth branches are absent: " + ", ".join(missing))
        arrays = tree.arrays(
    ...
    report = {
        "schema_version": "faser-xaod-truth-muon-kinematics-audit-v1",
