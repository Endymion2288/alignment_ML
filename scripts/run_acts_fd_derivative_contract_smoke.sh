#!/usr/bin/env bash
# Task B14L derivative-contract smoke.
# Pre-registered: 100043/0,1,37 (FD PASS controls) and 100048/86 (focus).
# Does not overwrite WB109–WB118 dumps and does not submit 1989 rows.
set -euo pipefail
...
WB118="$ROOT/outputs/leave_target_out_dump_v1/b14k_smoke/${SRC48}/ckf_leave_target_out_map_smoothness.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14l_smoke"
...
if [[ ! -f "$WB118" ]]; then
  echo "missing WB118 smoke dump: $WB118" >&2
  exit 2
fi
...
run_job() {
  ...
  echo "=== B14L derivative contract smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_derivative_contract.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14L derivative contract smoke complete"
