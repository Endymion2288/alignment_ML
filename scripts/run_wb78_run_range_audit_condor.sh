#!/usr/bin/env bash
# Condor CPU worker for Workbook 78 read-only run-range / data-contract audit.
...
python -u scripts/audit_wb78_run_range_data_contract.py --device cpu --with-w64
echo "=== Workbook 78 run-range data-contract audit completed successfully ==="
