#!/usr/bin/env python3
"""Submit Workbook-74 Physical Pair-Relative Route Encoder V1 source-transfer CV.
...
control and primary is ``route_representation_mode`` (absolute vs
physical_pair_relative).  Both are bounded at B=4.  No development /
final-blind / sealed data is touched.  Logs stay under the project output
tree.  Interactive lxplus GPUs are not used for the 30-epoch loop.

The GPU requirement targets the broad AlmaLinux9 GPU pool (V100/V100S/T4/A100/
H100/H200 + MIG via DeviceName), not just the most-contended H100/A100, so the
jobs schedule onto the freest available slots.
"""
...
WORKER = PROJECT_ROOT / "scripts" / "run_physical_pair_relative_route_v1_source_transfer_condor.sh"
# Broad GPU pool: accept any AlmaLinux9 GPU slot (V100/V100S/T4/A100/H100/H200
# + MIG via DeviceName), not just the most-contended H100/A100.  Capability
# >= 7.0 includes V100; the DeviceName regexp also catches the H100 MIG slots
# whose GPUs_Capability is undefined.
GPU_REQUIREMENT = (
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=str(
            PROJECT_ROOT
            / "outputs/mc24_four_station_physical_pair_relative_route_v1_source_transfer/training/condor"
        ),
    )
