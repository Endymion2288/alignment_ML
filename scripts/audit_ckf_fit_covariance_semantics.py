#!/usr/bin/env python3
"""Task B11 audit.  Official CKF fit-covariance semantics."""
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/ckf_fit_covariance_semantics_v1.yaml")
    ...
    store = ImmutableArtifactStore.begin(
        ...
        "sbb11_ckf_covariance_semantics",
    )
    ...
    store.write_json(
        "ckf_covariance_semantics.json",
        jsonable({"kind": "ckf_covariance_semantics", **common, **inventory["code_path"]}),
    )
    store.write_json(
        "track_parameter_provenance.json",
        jsonable(
            {"kind": "track_parameter_provenance", **common, **inventory["track_parameter_provenance"]}
        ),
    )
    store.write_json(
        "filtered_smoothed_state_audit.json",
        jsonable({"kind": "filtered_smoothed_state_audit", **common, **inventory["filtered_smoothed"]}),
    )
    store.write_json(
        "source_state_covariance_shape.json",
        jsonable({"kind": "source_state_covariance_shape", **common, **inventory["cin_shape"]}),
    )
    store.write_json("inherited_stage.json", {"kind": "inherited_stage", **inherited})
    store.write_json(
        "ckf_fit_covariance_semantics_decision.json",
        jsonable(
            {
                **common,
    ...
    print(
        f"SB-B11 verdict={decision['verdict']} case={decision['primary_case']} "
