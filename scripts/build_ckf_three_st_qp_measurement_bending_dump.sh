#!/usr/bin/env bash
# Compile the standalone C++ 3ST measurement-bending dump helper.
# Does not modify Calypso CMake.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "$script_dir/.." && pwd)"
src_dir="$project_root/alignment/three_st_qp_measurement_bending_dump"
out_dir="$project_root/build/three_st_qp_measurement_bending_dump"
...
flags = filtered[1:]
flags += [
    '-DATLAS_PACKAGE_NAME="CkfThreeStQpMeasurementBending"',
    "-DCkfThreeStQpMeasurementBending_EXPORTS",
    f"-I{src_dir}",
    f"-I{project_root}",
]
sources = [
    src_dir / "CkfThreeStQpMeasurementBendingAlg.cxx",
    src_dir / "CkfThreeStQpMeasurementBending_entries.cxx",
]
objects = []
...
so = out_dir / "libCkfThreeStQpMeasurementBending.so"
...
cat > "$out_dir/libCkfThreeStQpMeasurementBending.components" <<'EOF'
v2::libCkfThreeStQpMeasurementBending.so:CkfThreeStQpMeasurementBendingAlg
EOF
...
(
  cd "$out_dir"
  "$genconf_bin" -o "$out_dir/genConf" -p CkfThreeStQpMeasurementBending --no-init \
    -i libCkfThreeStQpMeasurementBending.so
)
...
printf '%s\n' "$part" > "$parts_file"
python "$merge_script" "$out_dir/CkfThreeStQpMeasurementBending.confdb2" "$parts_file"
printf 'Built C++ 3ST measurement-bending helper in %s\n' "$out_dir"
ls -l "$out_dir/libCkfThreeStQpMeasurementBending.so" \
  "$out_dir/libCkfThreeStQpMeasurementBending.components" \
  "$out_dir/CkfThreeStQpMeasurementBending.confdb2"
