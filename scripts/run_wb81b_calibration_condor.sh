#!/usr/bin/env bash
# Condor CPU worker for Workbook 81b: physics-constrained Arm D calibration.
# Does not retrain V5A / W64 / Arm B.  Does not expand hybrid or enter Arm C.
# Formal path is CPU for the same LCG / V100 / H100 VO reasons as WB79/80.
...
python -u scripts/train_eval_wb81b_calibration.py --fold "$fold" --device cpu
echo "=== Workbook 81b fold ${fold} completed successfully ==="
