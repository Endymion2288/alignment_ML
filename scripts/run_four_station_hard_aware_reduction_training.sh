#!/usr/bin/env bash
# Workbook 61: pre-registered hard-aware max-reduction V2 control.
# Development / transfer validation are not used to set weights, the
# reduction, or the OP.  Sealed test is never opened.  GPU-only.
# Do not compare this reduction against mean / top-k on transfer.
set -euo pipefail
...
CONTROL="$ROOT/outputs/mc24_four_station_hard_aware_reduction_v1"
...
train)
  python scripts/train_four_station_gauge_consistent_v2.py \
    --config configs/geometry_aware_transformer_v2_four_station_hard_aware_reduction.yaml \
    --objective-contract configs/physical_four_station_hard_aware_reduction_training.yaml \
...
compare)
  python scripts/evaluate_four_station_hard_aware_reduction_mechanism.py \
    --candidate-frozen-output "$CONTROL/checkpoint" \
    --control-frozen-output "$ROOT/outputs/mc24_four_station_dustbin_aware_route_v1/checkpoint" \
    --train-manifest "$TRAIN_OVERLAY" \
    --transfer-manifest "$TRANSFER/overlay_synthetic_v1/synthetic_corpus_manifest.json" \
    --gate-decision-json "$CONTROL/transfer_gate_decision.json" \
    --output-dir "$CONTROL/mechanism_comparison" \
    --device cuda \
    --batch-size 32
  ;;
...
assess)
  python scripts/evaluate_four_station_dustbin_aware_transfer.py \
    --gates configs/physical_four_station_hard_aware_reduction_training.yaml \
    --iteration-manifest "$TRANSFER/iteration_manifest.json" \
    --split validation \
