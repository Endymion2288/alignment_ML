#!/usr/bin/env bash
# Compile the standalone C++ 3ST→IFT prediction helper against the existing
...
src_dir="$project_root/alignment/three_st_to_ift_dump"
out_dir="$project_root/build/three_st_to_ift_dump"
...
flags = filtered[1:]
flags += [
    '-DATLAS_PACKAGE_NAME="CkfThreeStToIftPrediction"',
    "-DCkfThreeStToIftPrediction_EXPORTS",
    f"-I{src_dir}",
    f"-I{project_root}",
]
sources = [
    src_dir / "CkfThreeStToIftPredictionAlg.cxx",
    src_dir / "CkfThreeStToIftPrediction_entries.cxx",
]
objects = []
...
so = out_dir / "libCkfThreeStToIftPrediction.so"
...
cat > "$out_dir/libCkfThreeStToIftPrediction.components" <<'EOF'
v2::libCkfThreeStToIftPrediction.so:CkfThreeStToIftPredictionAlg
EOF
...
(
  cd "$out_dir"
  "$genconf_bin" -o "$out_dir/genConf" -p CkfThreeStToIftPrediction --no-init \
    -i libCkfThreeStToIftPrediction.so
)
...
printf '%s\n' "$part" > "$parts_file"
python "$merge_script" "$out_dir/CkfThreeStToIftPrediction.confdb2" "$parts_file"
printf 'Built C++ 3ST→IFT prediction helper in %s\n' "$out_dir"
ls -l "$out_dir/libCkfThreeStToIftPrediction.so" \
  "$out_dir/libCkfThreeStToIftPrediction.components" \
  "$out_dir/CkfThreeStToIftPrediction.confdb2"
