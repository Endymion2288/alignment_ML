#!/usr/bin/env bash
# Compile the standalone C++ 3ST q/p calibration dump helper against the
# existing Calypso/Athena build.  Does not modify Calypso CMake.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "$script_dir/.." && pwd)"
src_dir="$project_root/alignment/three_st_qp_calibration_dump"
out_dir="$project_root/build/three_st_qp_calibration_dump"
...
flags = filtered[1:]
flags += [
    '-DATLAS_PACKAGE_NAME="CkfThreeStQpCalibration"',
    "-DCkfThreeStQpCalibration_EXPORTS",
    f"-I{src_dir}",
    f"-I{project_root}",
]
sources = [
    src_dir / "CkfThreeStQpCalibrationAlg.cxx",
    src_dir / "CkfThreeStQpCalibration_entries.cxx",
]
objects = []
...
so = out_dir / "libCkfThreeStQpCalibration.so"
...
cat > "$out_dir/libCkfThreeStQpCalibration.components" <<'EOF'
v2::libCkfThreeStQpCalibration.so:CkfThreeStQpCalibrationAlg
EOF
...
(
  cd "$out_dir"
  "$genconf_bin" -o "$out_dir/genConf" -p CkfThreeStQpCalibration --no-init \
    -i libCkfThreeStQpCalibration.so
)
...
printf '%s\n' "$part" > "$parts_file"
python "$merge_script" "$out_dir/CkfThreeStQpCalibration.confdb2" "$parts_file"
printf 'Built C++ 3ST q/p calibration helper in %s\n' "$out_dir"
ls -l "$out_dir/libCkfThreeStQpCalibration.so" \
  "$out_dir/libCkfThreeStQpCalibration.components" \
  "$out_dir/CkfThreeStQpCalibration.confdb2"
