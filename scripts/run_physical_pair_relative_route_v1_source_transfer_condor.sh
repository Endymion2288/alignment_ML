#!/usr/bin/env bash
# Condor GPU worker for Workbook 74: Physical Pair-Relative Route Encoder V1
# train-side source-transfer CV head-only training.  Runs one (fold, arm) job.
# Each fold trains on the fold's TRAIN-family regenerated corpus only; the
# held-out family never enters training.
#
...
#   arm  = control          -> Workbook-72 V5A absolute BOUNDED B=4 (absolute node latent)
#   arm  = primary          -> Physical Pair-Relative Route Encoder BOUNDED B=4 (R_phys only)
set -euo pipefail
...
if not torch.cuda.is_available():
    raise SystemExit("Physical Pair-Relative source-transfer training is GPU-only; CUDA is unavailable on this slot")
...
train_root="outputs/mc24_four_station_physical_pair_relative_route_v1_source_transfer/training"
...
case "$arm" in
  control)
    config="configs/wb74_absolute_bounded_control_source_transfer.yaml"
    ;;
  primary)
    config="configs/wb74_physical_pair_relative_bounded_primary_source_transfer.yaml"
    ;;
  *)
...
echo "=== Physical Pair-Relative source-transfer training: fold=$fold arm=$arm ==="
echo "config=$config"
