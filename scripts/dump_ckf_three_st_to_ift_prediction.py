#!/usr/bin/env python3
"""Athena job: load the C++ 3ST→IFT prediction helper.
...
PLUGIN_DIR = ROOT / "build" / "three_st_to_ift_dump"
DEFAULT_CONFIG = ROOT / "configs" / "three_st_to_ift_prediction_v1.yaml"
MATERIAL_MAP = (
...
def _prepare_plugin() -> None:
    library = PLUGIN_DIR / "libCkfThreeStToIftPrediction.so"
    components = PLUGIN_DIR / "libCkfThreeStToIftPrediction.components"
    if not library.is_file() or not components.is_file():
        raise FileNotFoundError(
            "C++ 3ST→IFT helper is absent. "
            "Run scripts/build_ckf_three_st_to_ift_dump.sh first."
        )
...
def main() -> int:
    ...
    try:
        alg_factory = CompFactory.getComp("CkfThreeStToIftPredictionAlg")
    except Exception as exc:
        raise RuntimeError(
            "C++ helper is not visible to CompFactory. "
            "Rebuild scripts/build_ckf_three_st_to_ift_dump.sh"
        ) from exc
    ...
    def _make_tool(name: str, ms: bool, eloss: bool, record: bool):
        ...
        tool.MaxSteps = int(config["smoke"]["max_steps"])
    ...
    alg = alg_factory("CkfThreeStToIftPredictionAlg")
    ...
    alg.FourStationCollection = "CKFTrackCollection"
    alg.ClusterContainer = "SCT_ClusterContainer"
