#!/usr/bin/env bash
# Yasu Stage 0 smoke: 20 events on one construction and one validation source.
...
DUMP_ROOT="$ROOT/outputs/ckf_without_ift_definition_v1/dumps"
...
dump_one() {
  ...
  local tracks="$outdir/ckf_without_ift_definition.jsonl"
  local events="$outdir/ckf_without_ift_events.jsonl"
  mkdir -p "$outdir"
  if [[ -f "$tracks" || -f "$events" ]]; then
    echo "refusing to overwrite $outdir" >&2
    exit 2
  fi
  echo "=== WithoutIFT definition dump source=$source_id ==="
  (
    cd "$outdir"
    python "$ROOT/scripts/dump_ckf_without_ift_definition.py" \
      --input-xaod "$xaod" \
      --source-id "$source_id" \
      --output "$tracks" \
      --event-output "$events" \
...
echo "Yasu Stage 0 smoke dumps complete"
