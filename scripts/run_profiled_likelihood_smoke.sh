#!/usr/bin/env bash
# Task B14M measurement-only profile smoke.
# Pre-registered development events: 100043/0,1,37 and 100048/86.
# Does not overwrite WB109 dumps and does not choose a seed.
set -euo pipefail
...
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14m_smoke"
...
run_job() {
  ...
  echo "=== B14M profile smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_profile.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14M profile smoke complete"
