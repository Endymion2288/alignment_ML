#!/usr/bin/env bash
# Task B14X smoke.  Rebuilds the diagnostic helper, then dumps 0/1/37/86
# with field-gradient tangent repair.  Does not overwrite WB109–WB122 dumps.
set -euo pipefail
...
WB12043="$ROOT/outputs/leave_target_out_dump_v1/b14u_smoke/${SRC43}/ckf_leave_target_out_official_jacobian.jsonl"
WB109="$ROOT/outputs/leave_target_out_dump_v1/dumps/${SRC43}/ckf_leave_target_out.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14x_smoke"
...
if [[ ! -f "$WB12043" ]]; then
  echo "missing frozen WB120 dump: $WB12043" >&2
  exit 2
fi
...
run_job() {
  ...
  echo "=== B14X field-gradient smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
echo "B14X field-gradient variational repair smoke complete"
ls -l "$SMOKE_ROOT"/*/*.jsonl
python "$ROOT/scripts/run_field_gradient_variational_repair.py"
