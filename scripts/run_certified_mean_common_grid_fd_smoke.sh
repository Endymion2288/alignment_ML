#!/usr/bin/env bash
# Task B14ZC smoke.  Rebuilds the diagnostic helper, then dumps 100048/86
# certified-mean common-grid independent FD plus WB123 repaired tangent A.
# Reuses frozen WB123 100043 dumps for control 0/1/37.  Does not overwrite
# WB109–WB127 dumps.  Does not change FieldGradientDefaultExtension or
# WB127 mean semantics.  Does not enable WB124 DOPRI5 or the failed
# WB125 20/10/5 classical-RK4+Mean integrator as the official C.
set -euo pipefail
...
WB12748="$ROOT/outputs/leave_target_out_dump_v1/b14zb_smoke/mc24_100048_00000_00049/ckf_leave_target_out_surface_eloss.jsonl"
WB109="$ROOT/outputs/leave_target_out_dump_v1/dumps/mc24_100043_00400_00499/ckf_leave_target_out.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14zc_smoke"
...
if [[ ! -f "$WB12748" ]]; then
  echo "missing frozen WB127 dump: $WB12748" >&2
  exit 2
fi
...
echo "=== B14ZC certified-mean FD smoke source=$SRC48 events=86 out=$partfile ==="
(
  cd "$outdir"
...
final="$outdir/ckf_leave_target_out_certified_mean_fd.jsonl"
if [[ -f "$final" ]]; then
...
echo "B14ZC certified-mean common-grid FD smoke complete"
ls -l "$SMOKE_ROOT"/*/*.jsonl
echo "official audit is scripts/audit_certified_mean_common_grid_fd.py in the ML env"
