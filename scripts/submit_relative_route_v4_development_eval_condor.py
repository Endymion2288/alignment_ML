#!/usr/bin/env python3
"""Submit Workbook-70 Arm 0/1/2 reserved-blind development evaluation.

One GPU job.  Does not train, retune, or open unused final blind / sealed test.
Interactive lxplus GPUs are not used.
"""
...
WORKER = PROJECT_ROOT / "scripts" / "run_relative_route_v4_development_eval_condor.sh"
GPU_REQUIREMENT = (
...
def _write_submit(
    ...
    submit_file = submit_dir / "development_eval.sub"
    ...
    submit_file.write_text(
        "\n".join(
            (
                ...
                f"output = {log_dir}/eval.$(ClusterId).$(ProcId).out",
                f"error = {log_dir}/eval.$(ClusterId).$(ProcId).err",
                f"log = {log_dir}/eval.$(ClusterId).log",
...
def main() -> None:
    ...
    parser.add_argument(
        "--submit-dir",
        default=str(PROJECT_ROOT / "outputs/mc24_four_station_relative_route_v4_head_only/condor_eval"),
    )
