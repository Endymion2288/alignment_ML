#!/usr/bin/env python3
"""Reserved-blind mechanism audit for workbook 64.  Does not train or retune."""
...
from training.source_diversity_audit import RESERVED_BLIND_SOURCES, assert_sources_allowed
...
def main() -> None:
    ...
    if str(args.device) != "cuda":
        raise SystemExit("blind mechanism audit is GPU-only")
    ...
    if {sample.split for sample in samples} != {"validation"}:
        raise ValueError("blind mechanism audit must load the reserved-blind validation split")
    ...
    if constituents != set(RESERVED_BLIND_SOURCES):
        raise SystemExit("mechanism audit must use only the reserved blind pair")
    ...
    payload = {
        ...
        "reserved_blind_sources": sorted(constituents),
        "test_data_accessed": False,
    }
    _write_json(Path(args.output_json), payload)
    print(str(_json_ready(payload))[:4000])
