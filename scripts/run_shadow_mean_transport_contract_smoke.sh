#!/usr/bin/env bash
# Task B14ZA smoke.  Rebuilds the diagnostic helper, then dumps 100048/86
# required-hop production/shadow mean ledgers.  Reuses frozen WB123
# control 0/1/37 PASS.  Does not overwrite WB109–WB125 dumps.
# Does not change FieldGradientDefaultExtension.  Does not enable WB124
# DOPRI5 or WB125 common-grid FD.  Does not evaluate Jacobian.
set -euo pipefail
...
WB12548="$ROOT/outputs/leave_target_out_dump_v1/b14z_smoke/mc24_100048_00000_00049/ckf_leave_target_out_official_jacobian.jsonl"
WB109="$ROOT/outputs/leave_target_out_dump_v1/dumps/mc24_100043_00400_00499/ckf_leave_target_out.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14za_smoke"
...
if [[ ! -f "$WB12548" ]]; then
  echo "missing frozen WB125 dump: $WB12548" >&2
  exit 2
fi
...
echo "=== B14ZA shadow-mean-transport smoke source=$SRC48 events=86 out=$partfile ==="
(
  cd "$outdir"
...
final="$outdir/ckf_leave_target_out_shadow_mean.jsonl"
if [[ -f "$final" ]]; then
...
echo "B14ZA shadow mean transport smoke complete"
ls -l "$SMOKE_ROOT"/*/*.jsonl
echo "official audit is scripts/audit_shadow_mean_transport_contract.py in the ML env"
