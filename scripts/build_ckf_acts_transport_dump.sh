#!/usr/bin/env bash
# Compile the standalone C++ ACTS transport dump helper against the existing
# Calypso/Athena build.  Does not modify Calypso CMake or FaserActs source.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "$script_dir/.." && pwd)"
src_dir="$project_root/alignment/acts_transport_dump"
out_dir="$project_root/build/acts_transport_dump"
calypso_root="${CALYPSO_ROOT:-$project_root/../calypso}"
platform="${CALYPSO_PLATFORM:-x86_64-el9-gcc13-opt}"
...
python - "$compile_json" "$link_txt" "$src_dir" "$out_dir" "$project_root" <<'PY'
import json
import shlex
import sys
from pathlib import Path

compile_json, link_txt, src_dir, out_dir, project_root = map(Path, sys.argv[1:])
...
# Drop the original source / object pair and package export.
filtered = []
skip_next = False
...
filtered[0:0] = []  # keep compiler as first token
compiler = filtered[0]
flags = filtered[1:]
flags += [
    '-DATLAS_PACKAGE_NAME="CkfActsTransportDump"',
    "-DCkfActsTransportDump_EXPORTS",
    f"-I{src_dir}",
    f"-I{project_root}",
]
sources = [
    src_dir / "CkfActsTransportDumpAlg.cxx",
    src_dir / "CkfActsTransportDump_entries.cxx",
]
objects = []
for source in sources:
    ...
    Path(out_dir / "compile_one.sh").write_text(
        "#!/usr/bin/env bash\nset -euo pipefail\n" + shlex.join(cmd) + "\n",
        encoding="utf-8",
    )
    import subprocess
...
# Skip the atlas_build_run.sh wrapper and original objects / output.
gxx = None
libs = []
saw_shared = False
...
so = out_dir / "libCkfActsTransportDump.so"
...
cat > "$out_dir/libCkfActsTransportDump.components" <<'EOF'
v2::libCkfActsTransportDump.so:CkfActsTransportDumpAlg
EOF
...
(
  cd "$out_dir"
  "$genconf_bin" -o "$out_dir/genConf" -p CkfActsTransportDump --no-init \
    -i libCkfActsTransportDump.so
)
...
printf '%s\n' "$part" > "$parts_file"
python "$merge_script" "$out_dir/CkfActsTransportDump.confdb2" "$parts_file"
printf 'Built C++ ACTS transport dump helper in %s\n' "$out_dir"
ls -l "$out_dir/libCkfActsTransportDump.so" \
  "$out_dir/libCkfActsTransportDump.components" \
  "$out_dir/CkfActsTransportDump.confdb2"
