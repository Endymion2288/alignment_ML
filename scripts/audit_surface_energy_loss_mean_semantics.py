#!/usr/bin/env python3
"""Task B14ZB audit.  ACTS surface energy-loss mean semantics contract."""
...
def main() -> int:
    ...
    store = ImmutableArtifactStore.begin(
        ...
        "sbb14zb_surface_eloss_semantics",
    )
    ...
    store.write_json(
        ...
        jsonable(
            {
                "kind": "surface_energy_loss_node_ledger",
                **common,
                "targets": [
                    {
                        "target_station": item.get("target_station"),
                        ...
                        "node_qop_contract": item.get("node_qop_contract"),
                    }
                    for item in segs
                ],
            }
    ...
    store.write_json(
        ...
        jsonable(
            {
                "kind": "material_update_gating_contract",
                **common,
                "targets": [
                    {
                        "target_station": item.get("target_station"),
    ...
    store.finalize(
        {
            ...
            "config_sha256": config_sha,
            "inherited_wb126_decision_sha256": decision[
                "inherited_wb126_decision_sha256"
            ],
            ...
            "shadow_mean_contract_established": decision[
    ...
    print(
        f"SB-B14ZB verdict={decision['verdict']} case={decision['primary_case']} "
