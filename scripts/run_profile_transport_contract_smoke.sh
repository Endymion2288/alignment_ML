#!/usr/bin/env bash
# Task B14T measurement-transport smoke.
# Pre-registered development events: 100043/0,1,37 and 100048/86.
# Does not overwrite WB109, WB114, or WB115 dumps and does not submit
...
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14t_smoke"
...
run_job() {
  ...
  echo "=== B14T transport smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
    python "$ROOT/scripts/dump_ckf_leave_target_out.py" \
      ...
      --profile-diagnostic-max-steps 20000 \
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_profile_transport.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14T transport smoke complete"
