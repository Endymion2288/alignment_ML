#!/usr/bin/env bash
# Task B14J Jacobian-continuity smoke.
# Pre-registered development events: 100043/0,1,37 and 100048/86.
# Does not overwrite WB109, WB114, WB115, or WB116 dumps and does not
# submit the 1989-row campaign.
set -euo pipefail
...
WB114="$ROOT/outputs/leave_target_out_dump_v1/b14m_smoke/${SRC43}/ckf_leave_target_out_profile.jsonl"
WB115="$ROOT/outputs/leave_target_out_dump_v1/b14n_smoke/${SRC43}/ckf_leave_target_out_profile_numerics.jsonl"
WB116="$ROOT/outputs/leave_target_out_dump_v1/b14t_smoke/${SRC43}/ckf_leave_target_out_profile_transport.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14j_smoke"
...
if [[ ! -f "$WB114" ]]; then
  echo "missing WB114 smoke dump: $WB114" >&2
  exit 2
fi
if [[ ! -f "$WB115" ]]; then
  echo "missing WB115 smoke dump: $WB115" >&2
  exit 2
fi
if [[ ! -f "$WB116" ]]; then
  echo "missing WB116 smoke dump: $WB116" >&2
  exit 2
fi
...
run_job() {
  ...
  echo "=== B14J jacobian continuity smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_jacobian_continuity.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14J jacobian continuity smoke complete"
