#!/usr/bin/env bash
# Task B14M-R profile-likelihood smoke reopen.
# Pre-registered events: 100043/0,1,37 and 100048/86.
# Does not overwrite WB114-WB128 dumps or the legacy b14m_smoke root.
...
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14m_reopen_smoke"
LEGACY="$ROOT/outputs/leave_target_out_dump_v1/b14m_smoke"
...
if [[ "$SMOKE_ROOT" == "$LEGACY" ]]; then
  echo "refusing to write into legacy b14m_smoke" >&2
  exit 2
fi
...
run_job() {
  ...
  echo "=== B14M-R reopen smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_b14m_reopen.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14M-R reopen smoke complete"
