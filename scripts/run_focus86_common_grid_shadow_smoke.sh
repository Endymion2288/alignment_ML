#!/usr/bin/env bash
# Task B14Z smoke.  Rebuilds the diagnostic helper, then dumps 100048/86
# required-hop common-grid shadow reference.  Reuses frozen WB123 100043
# dumps for control 0/1/37.  Does not overwrite WB109–WB124 dumps.
# Does not change FieldGradientDefaultExtension.  Does not enable WB124
# DOPRI5.  Does not retune production stepTolerance or the track FD ladder.
set -euo pipefail
...
WB12448="$ROOT/outputs/leave_target_out_dump_v1/b14y_smoke/mc24_100048_00000_00049/ckf_leave_target_out_official_jacobian.jsonl"
WB109="$ROOT/outputs/leave_target_out_dump_v1/dumps/mc24_100043_00400_00499/ckf_leave_target_out.jsonl"
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14z_smoke"
...
if [[ ! -f "$WB12448" ]]; then
  echo "missing frozen WB124 dump: $WB12448" >&2
  exit 2
fi
...
echo "=== B14Z focus-86 common-grid-shadow smoke source=$SRC48 events=86 out=$partfile ==="
(
  cd "$outdir"
...
echo "B14Z focus-86 common-grid shadow smoke complete"
ls -l "$SMOKE_ROOT"/*/*.jsonl
echo "official audit is scripts/audit_focus86_common_grid_shadow.py in the ML env"
