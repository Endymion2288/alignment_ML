#!/usr/bin/env bash
# Yasu Stage 2 smoke: 20 events on one construction and one validation source.
...
DUMP_ROOT="$ROOT/outputs/three_st_qp_calibration_v1/dumps/smoke"
...
dump_one() {
  ...
  local tracks="$outdir/three_st_qp_calibration.jsonl"
  local events="$outdir/three_st_qp_events.jsonl"
  mkdir -p "$outdir"
  if [[ -f "$tracks" || -f "$events" ]]; then
    echo "refusing to overwrite $outdir" >&2
    exit 2
  fi
  echo "=== 3ST q/p calibration dump source=$source_id ==="
  (
    cd "$outdir"
    python "$ROOT/scripts/dump_ckf_three_st_qp_calibration.py" \
      --input-xaod "$xaod" \
      --source-id "$source_id" \
      --output "$tracks" \
      --event-output "$events" \
...
export LD_LIBRARY_PATH="$ROOT/build/three_st_qp_calibration_dump${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"

dump_one "$CONSTRUCTION_XAOD" "$CONSTRUCTION_ID"
dump_one "$VALIDATION_XAOD" "$VALIDATION_ID"
echo "Yasu Stage 2 smoke dumps complete"
