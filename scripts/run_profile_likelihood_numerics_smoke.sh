#!/usr/bin/env bash
# Task B14N numerical-contract smoke.
# Pre-registered development events: 100043/0,1,37 and 100048/86.
# Does not overwrite WB109 or WB114 dumps and does not submit the full sample.
set -euo pipefail
...
SMOKE_ROOT="$ROOT/outputs/leave_target_out_dump_v1/b14n_smoke"
...
run_job() {
  ...
  echo "=== B14N numerics smoke source=$source_id events=$events out=$partfile ==="
  (
    cd "$outdir"
...
for source_id in "$SRC43" "$SRC48"; do
  ...
  final="$outdir/ckf_leave_target_out_profile_numerics.jsonl"
  if [[ ! -d "$outdir" ]]; then
    continue
  fi
  if [[ -f "$final" ]]; then
...
echo "B14N numerics smoke complete"
