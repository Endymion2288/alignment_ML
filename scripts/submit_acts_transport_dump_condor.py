#!/usr/bin/env python3
"""Submit one HTCondor job per WB87 source for the C++ ACTS transport dump."""
...
from datasets.acts_transport_dump import dump_path_for_source, load_config, plugin_library_path

PROJECT_ROOT = project_root()
WORKER = PROJECT_ROOT / "scripts" / "run_acts_transport_dump_condor.sh"
...
def main() -> int:
    ...
    parser.add_argument("--submit-dir", default="outputs/acts_transport_dump_v1/condor")
    ...
    if not library.is_file():
        raise FileNotFoundError(f"C++ dump helper is absent: {library}")
    ...
    for split in ("construction_sources", "validation_sources"):
        for spec in config["mc_data"][split]:
            output = dump_path_for_source(config, spec["source_id"])
            output.parent.mkdir(parents=True, exist_ok=True)
            xaod = Path(spec["input_xaod"])
            if not xaod.is_file():
                raise FileNotFoundError(f"input xAOD is missing: {xaod}")
            if output.is_file():
                raise FileExistsError(f"refusing to overwrite dump: {output}")
            jobs.append(
    ...
    submit_file = submit_dir / "ckf_acts_transport_dump.sub"
    ...
    payload = {
        ...
        "helper": "CkfActsTransportDumpAlg",
