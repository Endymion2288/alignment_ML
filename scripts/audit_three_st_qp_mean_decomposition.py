#!/usr/bin/env python3
"""Yasu-S2F audit.  Decompose the 3ST q/p mean.  Never flips S2 or opens S3."""
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/three_st_qp_mean_decomposition_v1.yaml")
    ...
    store = ImmutableArtifactStore.begin(
        ...
        f"yasu_s2f_three_st_qp_mean_decomposition_{args.campaign}",
    )
    ...
    store.write_json(
        ...
        {
            "kind": "three_st_qp_mean_decomposition_preregistration",
            "created_utc": created,
    ...
    store.write_json(
        ...
        {
            "kind": "three_st_qp_mean_decomposition_inventory",
            "campaign": args.campaign,
            "construction": {
    ...
    store.write_json("three_st_qp_mean_decomposition_contract.json", decision)
    store.finalize(
        {
            ...
            "train_validation_direction_consistent": decision[
                "train_validation_direction_consistent"
            ],
            "config_sha256": config_sha,
            "code_sha": git_head_sha(project_root()),
            "three_st_qp_trusted_observable": False,
    ...
    print(
        ...
        "stable",
        decision["train_validation_direction_consistent"],
    )
    print(
        "trusted",
