#!/usr/bin/env python3
"""Task B14Z audit.  86 common-grid shadow segment reference."""
...
def main() -> int:
    ...
    store = ImmutableArtifactStore.begin(
        ...
        "sbb14z_focus86_common_grid_shadow",
    )
    ...
    common = {
        ...
        "shadow_integrator_sha256": shadow_sha,
    ...
    store.write_json(
        "production_mean_physical_effects.json",
        jsonable(
            {
                "kind": "production_mean_physical_effects",
                **common,
    ...
    store.write_json(
        "frozen_common_grid_meshes.json",
        jsonable(
            {
                "kind": "frozen_common_grid_meshes",
                **common,
                "targets": inventory["focus_segments"],
            }
        ),
    ...
    store.write_json(
        "shadow_mean_contract.json",
        jsonable(
            {
                "kind": "shadow_mean_contract",
                **common,
                "targets": inventory["focus_segments"],
            }
        ),
    ...
    store.write_json(
        "common_grid_segment_fd.json",
        jsonable(
            {
                "kind": "common_grid_segment_fd",
                **common,
                "targets": inventory["focus_segments"],
            }
        ),
    ...
    store.write_json(
        "shadow_grid_refinement.json",
        jsonable(
            {
                "kind": "shadow_grid_refinement",
                **common,
                "targets": inventory["focus_segments"],
            }
        ),
    ...
    store.write_json(
        "triple_comparison_repaired_productionfd_commongrid.json",
        jsonable(
            {
                "kind": "triple_comparison",
                **common,
                "A": "production repaired tangent",
                "B": "production EigenStepper frozen four-rung segment FD",
                "C": "independent common-grid shadow mean FD",
                "targets": inventory["focus_segments"],
            }
        ),
    )
    store.write_json(
        ...
        jsonable(
            {
                "kind": "control_0_1_37_gate_replay",
                **common,
    ...
    store.write_json(
        "focus86_common_grid_shadow_decision.json",
        jsonable(
            {
                **common,
    ...
    print(
        f"SB-B14Z verdict={decision['verdict']} case={decision['primary_case']} "
