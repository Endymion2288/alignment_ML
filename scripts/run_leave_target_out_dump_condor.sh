#!/usr/bin/env bash
# Task B13 worker: independent leave-target-out refit dump.
# Does not call KalmanFitterTool.fit, does not write /Tracker/Align,
# does not seed q/p from truth, and does not start a new source campaign.
...
if [[ -f "$output_jsonl" ]]; then
  echo "Refusing to overwrite existing LTO dump: $output_jsonl" >&2
  exit 2
fi
