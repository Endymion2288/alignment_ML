#!/usr/bin/env python3
"""Athena job: load the C++ 3ST q/p calibration dump helper.
...
into the fit.  This file is standalone so Athena does not import the
ML-stack ``alignment`` / ``datasets`` packages.
"""
...
PLUGIN_DIR = ROOT / "build" / "three_st_qp_calibration_dump"
DEFAULT_CONFIG = ROOT / "configs" / "three_st_qp_calibration_v1.yaml"
MATERIAL_MAP = (
...
def _prepare_plugin() -> None:
    library = PLUGIN_DIR / "libCkfThreeStQpCalibration.so"
    components = PLUGIN_DIR / "libCkfThreeStQpCalibration.components"
    if not library.is_file() or not components.is_file():
        raise FileNotFoundError(
            "C++ 3ST q/p calibration helper is absent. "
            "Run scripts/build_ckf_three_st_qp_calibration_dump.sh first."
        )
...
def main() -> int:
    ...
    try:
        alg_factory = CompFactory.getComp("CkfThreeStQpCalibrationAlg")
    except Exception as exc:
        raise RuntimeError(
            "C++ helper is not visible to CompFactory. "
            "Rebuild scripts/build_ckf_three_st_qp_calibration_dump.sh"
        ) from exc
    ...
    alg = alg_factory("CkfThreeStQpCalibrationAlg")
