# Operating Protocol V1 paper results summary
...
**Decision flow.** `MC transfer PASS → real-data association PASS → Station calibration REJECT → reduced calibration REJECT → residual/DQ monitoring PASS`

**Operating state.** `real_data_operating_mode=residual_dq_monitoring_only`,
...
### Layer 1. Frozen V2 association transfers from MC to real data

**Status:** PASS

**Supports.** Frozen V2 association is transferable from MC to 2024 r0022 data.  The empty 100-event selected graph is statistics-limited, and full-segment route acceptance recovers.

**Does not support.** That a recoverable association graph is a solvable station alignment problem, or that residual decrease is closure.

### Layer 2. Association usable is not alignment solvable

**Status:** REJECT_GEOMETRY_WRITE

**Supports.** The real-data Station Jacobian has a gauge-like dz weak direction.  The 14974 5-DoF weak direction coincides with the frozen C_dx→station leakage direction A.  Reduced {dy,rx,rz} is Fisher-identifiable but does not transfer between independent calibration runs.  Geometry write is rejected by identifiability and cross-level ambiguity, not by a conservative threshold.

**Does not support.** A numerical value of C_dx, a transferable station correction, or that a chi-squared drop proves the geometry has been corrected.

### Layer 3. Current-geometry frozen-V2 residual/DQ monitoring is stable

**Status:** PASS

**Supports.** Under current official geometry, frozen V2 residual/DQ monitoring is stable on independent r0022 runs.  The seven new runs are nominal.  There is no persistent dy/rx drift.  Run 14977 is insufficient statistics, not an alignment anomaly.

**Does not support.** A geometry update, a survey trigger, or that monitoring residuals can be inverted into an alignment payload.
...
residual/DQ monitoring.  Do not extract any alignment payload from the current self-nulling residuals.  Continue residual/DQ monitoring only.
